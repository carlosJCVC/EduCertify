...
import { renderDatatableEvents } from "../events/speakers-events";
import { showCreateSpeakerModal } from "../../Presentation/utils/speaker-modal.utils";

/**
 * Load datatable for speakers
 */
export const listSpeakersUsingDataTable = () => {
    ...
    const options = {
        ...
        ajax: `${import.meta.env.VITE_APP_URL}/admin/speakers/json`,
        initComplete: function (settings, json) {
            const searchElement = document.querySelector("#speakers-datatable_wrapper .form-control");
            searchElement.classList.remove("form-control-sm");

            const filterElement = document.querySelector("#speakers-datatable_wrapper .form-select");
    ...
    const datatable = new DataTable(ElementSelectors.SpeakerDatatable, options);
    speakerStore.setSpeakerDatatable(datatable);
}
...
const getDatatableButtons = () => {
    return [
        ...
        {
            text: '<i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Create New Speaker</span>',
            className: 'btn-new-speaker btn-primary rounded-lg mb-2 btn-new-record',
            id: 'btn-new-speaker',
            action: () => {
